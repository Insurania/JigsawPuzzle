...
PuzzlePath::PuzzlePath(const QPoint &start, const QPoint &end, const QRect &bounds, Jigsaw::TypeOfPiece typeOfPiece, const CustomPuzzlePath &customPath, bool noRandom)
    : m_start(start)
    , m_end(end)
    , m_bounds(bounds)
...
void PuzzlePath::setStart(const QPoint &start)
{
    m_start = start;
    calculateParameters();
    generatePath();
...
void PuzzlePath::setEnd(const QPoint &end)
{
    m_end = end;
    calculateParameters();
    generatePath();
...
void PuzzlePath::setBounds(const QRect &bounds)
{
    m_bounds = bounds;
    generatePath();
}

void PuzzlePath::setTypeOfPiece(Jigsaw::TypeOfPiece typeOfPiece, const CustomPuzzlePath &customPath, bool noRandom)
{
    m_typeOfPiece = typeOfPiece;
    m_customPath = customPath;
    m_noRandom = noRandom;
...
QPainterPath PuzzlePath::path() const
{
    return m_path;
}

QPoint PuzzlePath::start() const
{
    return m_start;
}

QPoint PuzzlePath::end() const
{
    return m_end;
}

QRect PuzzlePath::bounds() const
{
    return m_bounds;
}

Jigsaw::TypeOfPiece PuzzlePath::typeOfPiece() const
{
    return m_typeOfPiece;
}

bool PuzzlePath::creatingPathSuccessful() const
{
    return m_successful;
}

int PuzzlePath::typeOfPieceToInt() const
{
    return PuzzlePath::typeOfPieceToInt(m_typeOfPiece);
}

QString PuzzlePath::tooltip() const
{
    return PuzzlePath::tooltip(m_typeOfPiece);
}

Jigsaw::TypeOfPiece PuzzlePath::intToTypeOfPiece(int index)
{
    switch (index) {
    case (0):
...
int PuzzlePath::typeOfPieceToInt(Jigsaw::TypeOfPiece type)
{
    switch (type) {
...
QString PuzzlePath::tooltip(Jigsaw::TypeOfPiece type)
{
    switch (type) {
    case Jigsaw::TypeOfPiece::TRAPEZOID:
...
QPainterPath PuzzlePath::singleJigsawPiecePath(const QRect &outerBounds, const QRect &innerBounds, Jigsaw::TypeOfPiece typeOfPiece,
                                               int minForcedPaths, bool useRecommendedInnerBounds,
                                               const CustomPuzzlePath &customPath)
{
    ...
    QVector<PuzzlePath> jigsawPaths(4);
    QPainterPath path;
    bool debug = false;
    int collisions = 0;
    int unsuccessful = 0;
    ...
    do {
        ...
        jigsawPaths[0] = PuzzlePath(topLeft, topRight, topBounds, typeOfPiece, customPath);
        jigsawPaths[1] = PuzzlePath(topRight, bottomRight, rightBounds, typeOfPiece, customPath);
        jigsawPaths[2] = PuzzlePath(bottomRight, bottomLeft, bottomBounds, typeOfPiece, customPath);
        jigsawPaths[3] = PuzzlePath(bottomLeft, topLeft, leftBounds, typeOfPiece, customPath);
    ...
    if (emergencyCounter >= maxAttempts && collisions > 0) {
        ...
        path = PuzzlePath(topLeft, topRight, topBounds, Jigsaw::TypeOfPiece::TRAPEZOID).path();
        path.connectPath(PuzzlePath(topRight, bottomRight, rightBounds, Jigsaw::TypeOfPiece::TRAPEZOID).path());
        path.connectPath(PuzzlePath(bottomRight, bottomLeft, bottomBounds, Jigsaw::TypeOfPiece::TRAPEZOID).path());
        path.connectPath(PuzzlePath(bottomLeft, topLeft, leftBounds, Jigsaw::TypeOfPiece::TRAPEZOID).path());
    }
    else {
...
void PuzzlePath::calculateParameters()
{
...
bool PuzzlePath::generateTrapezoidPath()
{
    m_path.clear();
...
bool PuzzlePath::generateSimpleArcPath()
{
...
bool PuzzlePath::generateTriangleConnectionsPath()
{
...
bool PuzzlePath::generateSimpleCircleConnectionsPath()
{
...
bool PuzzlePath::generateStandardPath()
{
...
bool PuzzlePath::generateStandardFunnyPath()
{
...
bool PuzzlePath::generateCustomPath()
{
    CustomPuzzlePath customPathTemp;
    ...
    do {
        ...
        for (int i = 1; i < actualPathPoints.size(); ++i) {
            m_customPath.pathPoint(i).typeOfLine == CustomPuzzlePath::TypeOfLine::STRAIGHT
...
bool PuzzlePath::pathHasCollisions(const PuzzlePath &jigsawPath, const QVector<PuzzlePath> &collisionPaths)
{
...
QRect PuzzlePath::recommendedInnerBounds(const QRect &outerBounds, Jigsaw::TypeOfPiece typeOfPiece, int minForcedPaths)
{
...
QVector<double> PuzzlePath::calculateRecommendedInnerBoundsPercentage(QVector<Jigsaw::TypeOfPiece> types, QSize size, int startPercentage, int startPaths, const CustomPuzzlePath &customPath)
{
