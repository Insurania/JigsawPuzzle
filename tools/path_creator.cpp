...
PathCreator::PathCreator(QWidget *parent)
    : QWidget{parent}
    ...
    , m_menuButtonInsert(new QPushButton("在之前插入", m_menuButtonsWidget1))
    , m_menuComboBoxInsertBefore(new QComboBox(m_menuButtonsWidget1))
    , m_menuButtonRemove(new QPushButton("删除", m_menuButtonsWidget1))
    ...
    , m_testButton(new QPushButton("测试!", m_testWidget))
    , m_testCheckBox(new QCheckBox("计算质量 (较慢!)", m_testWidget))
    ...
    , m_menuButtonApply(new QPushButton("应用", m_menuButtonsWidget2))
    , m_menuButtonOpen(new QPushButton("打开", m_menuButtonsWidget2))
    , m_menuButtonSave(new QPushButton("保存", m_menuButtonsWidget2))
    ...
    , m_controlPointsRestrictionButton(QVector<QPushButton*>(0, nullptr))
{
    ...
    m_mainLayout->addWidget(m_leftSideWidget);
        ...
        m_leftSideLayout->addWidget(m_testWidget);
            ...
            QLabel* test1 = new QLabel("测试 1", m_testWidget);
            QLabel* test2 = new QLabel("测试 2", m_testWidget);
            QLabel* test3 = new QLabel("测试 3", m_testWidget);
            ...
            m_testCheckBox->setToolTip("您的创作对于拼图游戏找到有效路径的可能性有多大?\n"
                                                                                "值为0.5及以上使其更可能，值为0.0使其非常不可能。");
    ...
    CustomPuzzlePath::RandomOffset noOffset;
    CustomPuzzlePath::RandomOffset circularOffset(CustomPuzzlePath::TypeOfRandomOffset::CIRCULAR);

    CustomPuzzlePath::Restriction noRestriction;
    CustomPuzzlePath::Restriction fixedPosition(CustomPuzzlePath::TypeOfRestriction::FIXEDPOSITION);

    CustomPuzzlePath::PathPoint pathPointStart(START, noOffset, fixedPosition);
    CustomPuzzlePath::PathPoint pathPoint1(MIDDLE, circularOffset, noRestriction, controlPoint1, noOffset, noRestriction);
    CustomPuzzlePath::PathPoint pathPointEnd(END, noOffset, fixedPosition, controlPoint2, circularOffset, noRestriction);

    m_customJigsawPath = CustomPuzzlePath({pathPointStart, pathPoint1, pathPointEnd});
    addPathPoint(0, false, true);
    addPathPoint(1, false);
    addPathPoint(2, false);

    synchronizeWidgetsWithCustomPuzzlePath();
    updateInsertRemoveComboBoxes();
    repaintCanvas();

    QObject::connect(m_canvas, &CreatorCanvas::clicked, this, &PathCreator::clicked);
    QObject::connect(m_canvas, &CreatorCanvas::released, this, &PathCreator::released);
    QObject::connect(m_canvas, &CreatorCanvas::dragged, this, &PathCreator::dragged);

    QObject::connect(m_menuButtonInsert, &QPushButton::clicked, this, &PathCreator::insert);
    QObject::connect(m_menuButtonRemove, &QPushButton::clicked, this, &PathCreator::remove);
    QObject::connect(m_testButton, &QPushButton::clicked, this, &PathCreator::testButtonClicked);

    QObject::connect(m_menuButtonApply, &QPushButton::clicked, this, &PathCreator::applyButtonClicked);
}
...
void PathCreator::clicked(const QPoint &position)
{
...
void PathCreator::released()
{
    m_selectedPathPoint = -1;
    m_selectedControlPoint = -1;
}

void PathCreator::dragged(const QPoint &position)
{
...
void PathCreator::typeChanged()
{
    ...
    for (int i = 0; i < m_customJigsawPath.numberOfPoints(); ++i) {
        ...
        m_controlPointsXSpinBox[i - 1]->setEnabled(m_customJigsawPath.pathPoint(i).typeOfLine == CustomPuzzlePath::TypeOfLine::CURVE &&
                                                  (m_controlPointsRestrictionType[i - 1]->currentIndex() == 0 || m_controlPointsRestrictionType[i - 1]->currentIndex() == 2));
        m_controlPointsYSpinBox[i - 1]->setEnabled(m_customJigsawPath.pathPoint(i).typeOfLine == CustomPuzzlePath::TypeOfLine::CURVE &&
                                                  (m_controlPointsRestrictionType[i - 1]->currentIndex() == 0 || m_controlPointsRestrictionType[i - 1]->currentIndex() == 2));
...
void PathCreator::inputFromWidgets()
{
    for (int i = 0; i < m_customJigsawPath.numberOfPoints(); ++i) {
        ...
        m_customJigsawPath.setRandomOffsetPathPoint(i, static_cast<CustomPuzzlePath::TypeOfRandomOffset>(m_pointsRandomOffsetType[i]->currentIndex()),
        ...
        m_customJigsawPath.setRestrictionPathPoint(i, static_cast<CustomPuzzlePath::TypeOfRestriction>(m_pointsRestrictionType[i]->currentIndex()),
        ...
        m_customJigsawPath.setRandomOffsetControlPoint(i, static_cast<CustomPuzzlePath::TypeOfRandomOffset>(m_controlPointsRandomOffsetType[i - 1]->currentIndex()),
        ...
        m_customJigsawPath.setRestrictionControlPoint(i, static_cast<CustomPuzzlePath::TypeOfRestriction>(m_controlPointsRestrictionType[i - 1]->currentIndex()),
        ...
        m_customJigsawPath.setTypeOfLine(i, static_cast<CustomPuzzlePath::TypeOfLine>(m_linesType[i - 1]->currentIndex()));
...
void PathCreator::insert()
{
    addPathPoint(m_menuComboBoxInsertBefore->currentIndex() + 1);
}

void PathCreator::remove()
{
    removePathPoint(m_menuComboBoxRemoveAt->currentIndex() + 1);
}

void PathCreator::testButtonClicked()
{
    m_testResultLabel->setText("测试中...");
    ...
    PuzzlePath jigsawPath(m_customJigsawPath.pathPoint(0).position.toPoint(),
    ...
    if (m_testCheckBox->isChecked()) {
        ...
        sizePercentage = PuzzlePath::calculateRecommendedInnerBoundsPercentage({Jigsaw::TypeOfPiece::CUSTOM}, QSize(100, 100), round(sizePercentage * 100), 4, m_customJigsawPath).at(0);
    }
    else {
    ...
    QPainterPath path1 = PuzzlePath::singleJigsawPiecePath(outerBounds, innerBounds, Jigsaw::TypeOfPiece::CUSTOM, 4, false, m_customJigsawPath);
    QPainterPath path2 = PuzzlePath::singleJigsawPiecePath(outerBounds, innerBounds, Jigsaw::TypeOfPiece::CUSTOM, 4, false, m_customJigsawPath);
    QPainterPath path3 = PuzzlePath::singleJigsawPiecePath(outerBounds, innerBounds, Jigsaw::TypeOfPiece::CUSTOM, 4, false, m_customJigsawPath);
    ...
    m_testResultLabel->setText(m_testCheckBox->isChecked() ? "完成! (质量: " + QString::number(sizePercentage) + ")" : "完成!");
}

void PathCreator::applyButtonClicked()
{
    emit apply(m_customJigsawPath);
}

void PathCreator::repaintCanvas()
{
    ...
    for (int i = 0; i < m_customJigsawPath.numberOfPoints(); ++i) {
        ...
        case CustomPuzzlePath::TypeOfRandomOffset::NONE: {
            break;
        }
        case CustomPuzzlePath::TypeOfRandomOffset::CIRCULAR: {
        ...
        case CustomPuzzlePath::TypeOfRandomOffset::LINE: {
            double value = 1.0 * m_customJigsawPath.pathPoint(i).randomOffset.value * distanceStartEnd / 100;
        ...
        if (i == 0 || m_customJigsawPath.pathPoint(i).typeOfLine == CustomPuzzlePath::TypeOfLine::STRAIGHT) continue;
...
void PathCreator::updateWidgetsValue()
{
    for (int i = 0; i < m_customJigsawPath.numberOfPoints(); ++i) {
...
void PathCreator::synchronizeWidgetsWithCustomPuzzlePath()
{
    m_signalBlocker.blockSignals();
    ...
    for (int i = 0; i < m_customJigsawPath.numberOfPoints(); ++i) {
        ...
        m_pointsLabel[i]->setText("点 " + QString::number(i) + ":");
        m_pointsXSpinBox[i]->setValue(m_customJigsawPath.pathPoint(i).position.x());
        m_pointsYSpinBox[i]->setValue(m_customJigsawPath.pathPoint(i).position.y());
        m_pointsXSpinBox[i]->setEnabled(m_customJigsawPath.pathPoint(i).restriction.type == CustomPuzzlePath::TypeOfRestriction::NONE ||
                                        m_customJigsawPath.pathPoint(i).restriction.type == CustomPuzzlePath::TypeOfRestriction::LINE);
        m_pointsYSpinBox[i]->setEnabled(m_customJigsawPath.pathPoint(i).restriction.type == CustomPuzzlePath::TypeOfRestriction::NONE ||
                                        m_customJigsawPath.pathPoint(i).restriction.type == CustomPuzzlePath::TypeOfRestriction::LINE);

        m_pointsRandomOffsetType[i]->setCurrentIndex(static_cast<int>(m_customJigsawPath.pathPoint(i).randomOffset.type));
        m_pointsRandomOffsetType[i]->setEnabled(m_customJigsawPath.pathPoint(i).restriction.type == CustomPuzzlePath::TypeOfRestriction::NONE ||
                                                m_customJigsawPath.pathPoint(i).restriction.type == CustomPuzzlePath::TypeOfRestriction::LINE);
        m_pointsRandomOffsetValue[i]->setValue(m_customJigsawPath.pathPoint(i).randomOffset.value);
        m_pointsRandomOffsetValue[i]->setEnabled(m_customJigsawPath.pathPoint(i).randomOffset.type != CustomPuzzlePath::TypeOfRandomOffset::NONE);
        ...
        m_pointsRestrictionL1P1[i]->setEnabled(m_customJigsawPath.pathPoint(i).restriction.type == CustomPuzzlePath::TypeOfRestriction::LINE ||
                                               m_customJigsawPath.pathPoint(i).restriction.type == CustomPuzzlePath::TypeOfRestriction::INTERSECTIONPOINT);
        m_pointsRestrictionL1P2[i]->setEnabled(m_customJigsawPath.pathPoint(i).restriction.type == CustomPuzzlePath::TypeOfRestriction::LINE ||
                                               m_customJigsawPath.pathPoint(i).restriction.type == CustomPuzzlePath::TypeOfRestriction::INTERSECTIONPOINT);
        m_pointsRestrictionL2P1[i]->setEnabled(m_customJigsawPath.pathPoint(i).restriction.type == CustomPuzzlePath::TypeOfRestriction::INTERSECTIONPOINT);
        m_pointsRestrictionL2P2[i]->setEnabled(m_customJigsawPath.pathPoint(i).restriction.type == CustomPuzzlePath::TypeOfRestriction::INTERSECTIONPOINT);
        m_pointsRestrictionDistanceSpinBox[i]->setValue(m_customJigsawPath.pathPoint(i).restriction.distanceFromLine);
        m_pointsRestrictionDistanceSpinBox[i]->setEnabled(m_customJigsawPath.pathPoint(i).restriction.type == CustomPuzzlePath::TypeOfRestriction::LINE);

        if (i == 0) continue;

        m_linesLabel[i - 1]->setText("线 " + QString::number(i) + ":");
        m_linesType[i - 1]->setCurrentIndex(static_cast<int>(m_customJigsawPath.pathPoint(i).typeOfLine));

        m_controlPointsLabel[i - 1]->setText("控制点 " + QString::number(i) + ":");
        m_controlPointsXSpinBox[i - 1]->setValue(m_customJigsawPath.pathPoint(i).positionControlPoint.x());
        m_controlPointsYSpinBox[i - 1]->setValue(m_customJigsawPath.pathPoint(i).positionControlPoint.y());
        m_controlPointsXSpinBox[i - 1]->setEnabled(m_customJigsawPath.pathPoint(i).typeOfLine == CustomPuzzlePath::TypeOfLine::CURVE &&
                                                  (m_customJigsawPath.pathPoint(i).restrictionControlPoint.type == CustomPuzzlePath::TypeOfRestriction::NONE ||
                                                   m_customJigsawPath.pathPoint(i).restrictionControlPoint.type == CustomPuzzlePath::TypeOfRestriction::LINE));
        m_controlPointsYSpinBox[i - 1]->setEnabled(m_customJigsawPath.pathPoint(i).typeOfLine == CustomPuzzlePath::TypeOfLine::CURVE &&
                                                  (m_customJigsawPath.pathPoint(i).restrictionControlPoint.type == CustomPuzzlePath::TypeOfRestriction::NONE ||
                                                   m_customJigsawPath.pathPoint(i).restrictionControlPoint.type == CustomPuzzlePath::TypeOfRestriction::LINE));
        ...
        m_controlPointsRandomOffsetType[i - 1]->setEnabled(m_customJigsawPath.pathPoint(i).restrictionControlPoint.type == CustomPuzzlePath::TypeOfRestriction::NONE ||
                                                           m_customJigsawPath.pathPoint(i).restrictionControlPoint.type == CustomPuzzlePath::TypeOfRestriction::LINE);
        m_controlPointsRandomOffsetValue[i - 1]->setValue(m_customJigsawPath.pathPoint(i).randomOffsetControlPoint.value);
        m_controlPointsRandomOffsetValue[i - 1]->setEnabled(m_customJigsawPath.pathPoint(i).randomOffsetControlPoint.type != CustomPuzzlePath::TypeOfRandomOffset::NONE);
        ...
        m_controlPointsRestrictionType[i - 1]->setEnabled(m_customJigsawPath.pathPoint(i).typeOfLine == CustomPuzzlePath::TypeOfLine::CURVE);
        ...
        m_controlPointsRestrictionL1P1[i - 1]->setEnabled(m_customJigsawPath.pathPoint(i).restrictionControlPoint.type == CustomPuzzlePath::TypeOfRestriction::LINE ||
                                                          m_customJigsawPath.pathPoint(i).restrictionControlPoint.type == CustomPuzzlePath::TypeOfRestriction::INTERSECTIONPOINT);
        m_controlPointsRestrictionL1P2[i - 1]->setEnabled(m_customJigsawPath.pathPoint(i).restrictionControlPoint.type == CustomPuzzlePath::TypeOfRestriction::LINE ||
                                                          m_customJigsawPath.pathPoint(i).restrictionControlPoint.type == CustomPuzzlePath::TypeOfRestriction::INTERSECTIONPOINT);
        m_controlPointsRestrictionL2P1[i - 1]->setEnabled(m_customJigsawPath.pathPoint(i).restrictionControlPoint.type == CustomPuzzlePath::TypeOfRestriction::INTERSECTIONPOINT);
        m_controlPointsRestrictionL2P2[i - 1]->setEnabled(m_customJigsawPath.pathPoint(i).restrictionControlPoint.type == CustomPuzzlePath::TypeOfRestriction::INTERSECTIONPOINT);
        ...
        m_controlPointsRestrictionDistanceSpinBox[i - 1]->setEnabled(m_customJigsawPath.pathPoint(i).restrictionControlPoint.type == CustomPuzzlePath::TypeOfRestriction::LINE);
    }

    m_pointsRestrictionType.last()->setEnabled(false);
    m_pointsRestrictionType[0]->setEnabled(false);
    m_pointsLabel.last()->setText("终点:");
    m_pointsLabel[0]->setText("起点:");

    m_signalBlocker.unblockSignals();
}

void PathCreator::updateInsertRemoveComboBoxes()
{
...
QPointF *PathCreator::qStringToQPointFPointer(const QString &text)
{
    if (text.isEmpty()) return nullptr;
    else if (text == "起点") return m_customJigsawPath.positionPathPoint(0);
    else if (text == "终点") return m_customJigsawPath.positionPathPoint(m_customJigsawPath.numberOfPoints() - 1);
...
QString PathCreator::qPointFPointerToQString(QPointF *point)
{
    for (int i = 0; i < m_customJigsawPath.numberOfPoints(); ++i) {
        if (m_customJigsawPath.positionPathPoint(i) == point) {
                    if (i == 0) return "起点";
        else if (i == m_customJigsawPath.numberOfPoints() - 1) return "终点";
        else return "点 " + QString::number(i);
        }
        if (i == 0) continue;
        if (m_customJigsawPath.positionControlPoint(i) == point) return "控制点 " + QString::number(i);
    }
    return "";
}

void PathCreator::addPathPoint(int index, bool calculateValues, bool isStartingPoint)
{
    ...
    if (calculateValues) {
        ...
        CustomPuzzlePath::RandomOffset offsetPoint(CustomPuzzlePath::TypeOfRandomOffset::CIRCULAR, 0.0,
                                                   m_customJigsawPath.positionPathPoint(index - 1),
                                                   m_customJigsawPath.positionPathPoint(index));
        CustomPuzzlePath::Restriction noRestriction(CustomPuzzlePath::TypeOfRestriction::NONE,
                                                    m_customJigsawPath.positionPathPoint(index - 1),
                                                    m_customJigsawPath.positionControlPoint(index - 1),
                                                    m_customJigsawPath.positionPathPoint(index),
                                                    m_customJigsawPath.positionControlPoint(index));

        CustomPuzzlePath::PathPoint newPathPoint(newPosition, offsetPoint, noRestriction, newPositionControlPoint, offsetPoint, noRestriction);
...
void PathCreator::removePathPoint(int index)
{
...
void PathCreator::addPointToWidget()
{
    ...
    QLabel* xLabel = new QLabel("X坐标:", m_entriesWidget.last());
    ...
    QObject::connect(m_pointsXSpinBox.last(), &QDoubleSpinBox::valueChanged, this, &PathCreator::inputFromWidgets);
    m_signalBlocker.add(m_pointsXSpinBox.last());

    QLabel* yLabel = new QLabel("Y坐标:", m_entriesWidget.last());
    ...
    QObject::connect(m_pointsYSpinBox.last(), &QDoubleSpinBox::valueChanged, this, &PathCreator::inputFromWidgets);
    m_signalBlocker.add(m_pointsYSpinBox.last());

    QLabel* offsetTypeLabel = new QLabel("偏移类型:", m_entriesWidget.last());
    m_entriesLayout.last()->addWidget(offsetTypeLabel);
    m_pointsRandomOffsetType.push_back(new QComboBox(m_entriesWidget.last()));
    m_pointsRandomOffsetType.last()->addItems({"无", "圆形", "直线"});
    m_entriesLayout.last()->addWidget(m_pointsRandomOffsetType.last());
    QObject::connect(m_pointsRandomOffsetType.last(), &QComboBox::currentIndexChanged, this, &PathCreator::typeChanged);
    m_signalBlocker.add(m_pointsRandomOffsetType.last());

    QLabel* valueOffsetLabel = new QLabel("数值", m_entriesWidget.last());
    ...
    QObject::connect(m_pointsRandomOffsetValue.last(), &QDoubleSpinBox::valueChanged, this, &PathCreator::inputFromWidgets);
    m_signalBlocker.add(m_pointsRandomOffsetValue.last());

    QLabel* offsetLineLabel = new QLabel("直线:", m_entriesWidget.last());
    ...
    m_pointsRandomOffsetButton.push_back(new QPushButton("应用", m_entriesWidget.last()));
    m_entriesLayout.last()->addWidget(m_pointsRandomOffsetButton.last());
    QObject::connect(m_pointsRandomOffsetButton.last(), &QPushButton::clicked, this, &PathCreator::inputFromWidgets);
     m_signalBlocker.add(m_pointsRandomOffsetButton.last());
}

void PathCreator::addPointRestrictionToWidget()
{
    m_entriesWidget.push_back(new QWidget(m_rightSideWidget));
    m_rightSideLayout->addWidget(m_entriesWidget.last());
    m_entriesLayout.push_back(new QHBoxLayout(m_entriesWidget.last()));
    ...
    m_pointsRestrictionLabel.push_back(new QLabel("限制:", m_entriesWidget.last()));
    ...
    m_pointsRestrictionType.last()->addItems({"无", "固定", "直线", "交点"});
    m_entriesLayout.last()->addWidget(m_pointsRestrictionType.last());
    QObject::connect(m_pointsRestrictionType.last(), &QComboBox::currentIndexChanged, this, &PathCreator::typeChanged);
    m_signalBlocker.add(m_pointsRestrictionType.last());

    QLabel* line1Label = new QLabel("直线 1:", m_entriesWidget.last());
    ...
    QLabel* line2Label = new QLabel("直线 2:", m_entriesWidget.last());
    ...
    QLabel* distanceLabel = new QLabel("距离:", m_entriesWidget.last());
    ...
    QObject::connect(m_pointsRestrictionDistanceSpinBox.last(), &QDoubleSpinBox::valueChanged, this, &PathCreator::inputFromWidgets);
    m_signalBlocker.add(m_pointsRestrictionDistanceSpinBox.last());

    m_pointsRestrictionButton.push_back(new QPushButton("应用", m_entriesWidget.last()));
    m_entriesLayout.last()->addWidget(m_pointsRestrictionButton.last());
    QObject::connect(m_pointsRestrictionButton.last(), &QPushButton::clicked, this, &PathCreator::inputFromWidgets);
    m_signalBlocker.add(m_pointsRestrictionButton.last());
}

void PathCreator::addLineToWidget()
{
    m_entriesWidget.push_back(new QWidget(m_rightSideWidget));
    m_rightSideLayout->addWidget(m_entriesWidget.last());
    m_entriesLayout.push_back(new QHBoxLayout(m_entriesWidget.last()));
    ...
    QLabel* typeLabel = new QLabel("类型:", m_entriesWidget.last());
    ...
    m_linesType.last()->addItems({"直线", "曲线"});
    m_entriesLayout.last()->addWidget(m_linesType.last());
    QObject::connect(m_linesType.last(), &QComboBox::currentIndexChanged, this, &PathCreator::inputFromWidgets);
    m_signalBlocker.add(m_linesType.last());

    m_entriesLayout.last()->addSpacing(TABWIDTH * 10);
}

void PathCreator::addControlPointToWidget()
{
    m_entriesWidget.push_back(new QWidget(m_rightSideWidget));
    m_rightSideLayout->addWidget(m_entriesWidget.last());
    m_entriesLayout.push_back(new QHBoxLayout(m_entriesWidget.last()));
    ...
    QObject::connect(m_controlPointsXSpinBox.last(), &QDoubleSpinBox::valueChanged, this, &PathCreator::inputFromWidgets);
    ...
    QObject::connect(m_controlPointsYSpinBox.last(), &QDoubleSpinBox::valueChanged, this, &PathCreator::inputFromWidgets);
    ...
    m_controlPointsRandomOffsetType.last()->addItems({"无", "圆形", "直线"});
    m_entriesLayout.last()->addWidget(m_controlPointsRandomOffsetType.last());
    QObject::connect(m_controlPointsRandomOffsetType.last(), &QComboBox::currentIndexChanged, this, &PathCreator::typeChanged);
    ...
    QObject::connect(m_controlPointsRandomOffsetValue.last(), &QDoubleSpinBox::valueChanged, this, &PathCreator::inputFromWidgets);
    ...
    m_controlPointsRandomOffsetButton.push_back(new QPushButton("应用", m_entriesWidget.last()));
    m_entriesLayout.last()->addWidget(m_controlPointsRandomOffsetButton.last());
    QObject::connect(m_controlPointsRandomOffsetButton.last(), &QPushButton::clicked, this, &PathCreator::inputFromWidgets);
    m_signalBlocker.add(m_controlPointsRandomOffsetButton.last());
}

void PathCreator::addControlPointRestrictionToWidget()
{
    m_entriesWidget.push_back(new QWidget(m_rightSideWidget));
    m_rightSideLayout->addWidget(m_entriesWidget.last());
    m_entriesLayout.push_back(new QHBoxLayout(m_entriesWidget.last()));
    ...
    m_controlPointsRestrictionLabel.push_back(new QLabel("限制:", m_entriesWidget.last()));
    ...
    m_controlPointsRestrictionType.last()->addItems({"无", "固定", "直线", "交点"});
    m_entriesLayout.last()->addWidget(m_controlPointsRestrictionType.last());
    QObject::connect(m_controlPointsRestrictionType.last(), &QComboBox::currentIndexChanged, this, &PathCreator::typeChanged);
    ...
    QObject::connect(m_controlPointsRestrictionDistanceSpinBox.last(), &QDoubleSpinBox::valueChanged, this, &PathCreator::inputFromWidgets);
    m_signalBlocker.add(m_controlPointsRestrictionDistanceSpinBox.last());

    m_controlPointsRestrictionButton.push_back(new QPushButton("应用", m_entriesWidget.last()));
    m_entriesLayout.last()->addWidget(m_controlPointsRestrictionButton.last());
    QObject::connect(m_controlPointsRestrictionButton.last(), &QPushButton::clicked, this, &PathCreator::inputFromWidgets);
